...
module Lcov
  def lcov(compiler, src_dir, build_dir)
    ...
    genhtml_flags = "./lcov.output -o lcov-html --demangle-cpp --function-coverage --rc genhtml_hi_limit=#{compiler[:coverage_pass_limit]}  --rc genhtml_med_limit=#{compiler[:coverage_warn_limit]}"
    out, err, result = run_script(
